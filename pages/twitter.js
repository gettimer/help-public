...
import { createHelp, groupByCategory, getHelpsToolsNames } from "../utils/editdata";
import { fetchAPI } from "../lib/api";
import styles from "../styles/pages/layout.module.scss";
import { useRef, useState, useEffect } from "react";
import CopyLink from "../components/copylink";

function Twitter({ pages, twitterTool, navigationData, tools }) {
    ...
    return (
        <>
            <Head>
                <title>Twitter Tool Help</title>
                <link rel='icon' href='/favicon.ico' />
                <meta name="description" content='Circleboom Twitter Tool Help' />
            </Head>
            ...
            <main className={styles.wrapper}>
                <div ref={nav} className={`${styles.nav} ${isOpenNav && styles.nav_open}`}>
                    <SideMenu data={navigationData} activeItemTool={"twitter"} callback={resetMenu} tools={tools}/>
                </div>
                <section className={styles.section}>
                    ...
                    <div className={styles.list}>
                        {Object.entries(twitterTool).map(([key, value]) => (
                            <div key={key}>
                                <h3>{capitalizeStr(key)}</h3>
                                <ul>
                                    {value.map((ele) => (
                                        <li className={styles.list_item} key={ele.url}>
                                            <Link href={ele.url}>
                                                <a>
                                                    <span>
                                                        <Icon name='book' />
                        ...
                        {/* <div className={`${styles.hint} ${styles.hint_info}`}>Denneme dedmnemdnenm ndnmendmend</div>
                        <div className={`${styles.hint} ${styles.hint_warning}`}>Denneme dedmnemdnenm ndnmendmend</div>
                        <div className={`${styles.hint} ${styles.hint_danger}`}>Denneme dedmnemdnenm ndnmendmend</div> */}
                    </div>
                </section>
...
export async function getStaticProps() {
    ...
    allPages.map((ele) => {
        ele.Tool === "twitter" ? twitterToolHelps.push(createHelp(ele)) : null;
    });

    const unorderedTwitterToolOrder = groupByCategory(twitterToolHelps);
    twitterToolOrder = Object.keys(unorderedTwitterToolOrder)
        .reverse()
        .reduce((obj, key) => {
            obj[key] = unorderedTwitterToolOrder[key].sort((a, b) => b.id - a.id);
            return obj;
        }, {});
    ...
    const tools = getHelpsToolsNames(allPages)
